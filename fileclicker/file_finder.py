...
def pathlike_iter(L):
    """
    Iterate over substrings that satisify the following conditioons:
    * the previos char is start of line or one of DELIMITERS
    * when the first char is '/', the previous char is not '~'
    * the next char is either end of line or one of DELIMITERS
    * all of the sub-substirngs split by '/' does not exceed MAX_FNAME
    ...
    * a maximal (any string extending it does not satisfy all the above conditions)
    ...
    for i in range(len(L)):
        if i == 0 or PAT_DELIMITER_RE.match(L[i - 1]):
            ...
            if m:
                p = m.group(0)
                if p and not PAT_WHITESPACE_RE.match(p) and \
                        (not p.startswith('/') or i == 0 or L[i - 1] != '~') and \
                        not p.startswith(' ') and p.find('//') < 0:
                    yield i, p
